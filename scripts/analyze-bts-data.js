...
function analyzeDataPatterns(headers, rows) {
  ...
  headers.forEach((header, colIndex) => {
    ...
    if (header.toLowerCase().includes('price') || header.toLowerCase().includes('amount')) {
      patterns[header].businessType = 'currency';
    } else if (header.toLowerCase().includes('date') || header.toLowerCase().includes('time')) {
      patterns[header].businessType = 'datetime';
    } else if (header.toLowerCase().includes('status')) {
      patterns[header].businessType = 'status';
      patterns[header].statusValues = [...new Set(columnData)];
    } else if (header.toLowerCase().includes('id')) {
      patterns[header].businessType = 'identifier';
    }
  });
