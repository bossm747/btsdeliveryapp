...
export function CustomerPageWrapper({
  ...
  const handleRefresh = useCallback(async () => {
    if (onRefresh) {
      ...
      // Default: invalidate common customer queries
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["/api/customer/profile"] }),
        queryClient.invalidateQueries({ queryKey: ["/api/customer/orders/recent"] }),
  ...
  return (
    <ErrorBoundary>
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-[#004225] focus:text-white focus:rounded-md"
      >
