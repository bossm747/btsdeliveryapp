...
interface RiderPageWrapperProps {
  children: ReactNode;
...
export function RiderPageWrapper({
  children,
  refreshQueryKeys = [],
  onRefresh,
  ...
  const handleRefresh = useCallback(async () => {
    if (onRefresh) {
      ...
      // Default: invalidate common rider queries
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ["/api/rider/deliveries/history"] }),
        ...
        queryClient.invalidateQueries({ queryKey: ["/api/rider/performance"] }),
      ]);
    }
  ...
  return (
    <ErrorBoundary>
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-[#FF6B35] focus:text-white focus:rounded-md"
      >
      ...
      <div
        ...
        className="sr-only"
      >
        {isRefreshing && "Refreshing rider dashboard content..."}
      </div>
...
export default RiderPageWrapper;
